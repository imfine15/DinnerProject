...
<script>
	$.ajax({
		type: "get",
		url: "/semiproject/selectEnpimg.im",
		data: {
			enpNo:"<%=enpNo%>"
		},
		success: function(data){
			$("#eImg").attr("src", "<%=request.getContextPath()%>/thumbnail_uploadFile/"+data);
		},
		error: function(){
...
<body>
	...
	<form action="<%=request.getContextPath()%>/reservation.me"
		...
		<div
			...
			<div class="subb">
				<div style="width: 100%; margin-right: 0px;">
					<label style="font-size: 28px;" id="eName"></label><br> <br>
					<label style="color: gray; font-size: 20px;" id="eAddress">
					</label>
					<p style="width: 300px;" id="eIntro"></p>
					<br>
					<hr style="margin-left: -30px; margin-right: 50px;">
					<br> <input id="information" type="checkbox"
						style="width: 17px; height: 17px;"> <label
						style="font-size: 20px">이용자 전체 약관 동의</label> <a href=""><label
			...
			<div class="subb"
				...
				<label id="year"></label> <label id="month"></label> <label id="day"></label>
				<label id="dayLabel"></label> <input type="hidden" id="year1"
					name="year"> <input type="hidden" id="month1" name="month">
				<input type="hidden" id="day1" name="day"> <br> <br>
				<hr style="margin-right: -150px; margin-left: 0px;">
				<br> <label style="font-size: 20px; padding-right: 30px;">시간</label>
				<select name="hour" id="hour" class="sle"
					style="margin-right: 20px;">
					<option value="17">17시</option>
					<option value="18">18시</option>
					<option value="19">19시</option>
				</select> <select name="min" id="min" class="sle">
				...
				<hr style="margin-right: -150px; margin-left: 0px;">
				<br> <label style="font-size: 20px; padding-right: 30px;">인원</label>
				<label style="font-size: 12px;">성인</label> <select name="adult"
					id="adult" class="sle2" style="margin-right: 20px;">

					<%for(int i = 1; i < 101; i ++){ %>
					<option value="<%=i%>"><%=i %>명
					</option>
					<%} %>
				</select> <label style="font-size: 12px;">어린이</label> <select id="child"
					class="sle2" name="child">
					<%for(int i = 0; i < 101; i ++){ %>
					<option value="<%=i%>"><%=i %>명
					</option>
					<%} %>
				...
				<label style="margin-right: 30px; font-size: 20px;">일정</label><label
					id="finaltime"></label><br>
				<br> <label
					style="margin-right: 30px; margin-top: 10px; font-size: 20px;">인원</label><label
					id="people"></label> <br>
				<br>
				<hr style="margin-right: -150px; margin-left: 0px;">
				<label
					style="margin-right: 150px; margin-top: 10px; font-size: 20px;">요청사항</label>
				<textarea cols=40 rows=10 style="resize: none;"
					placeholder="요청사항을 입력해 주세요." name="rcontent"></textarea>
				<br>
				<br> <label>보유포인트 : </label><label id="currentPoint"></label> <br><br>
				<label>사용하실 포인트 : </label><input
					style="background: white; border: 1px solid black; border-radius: 3px;"
					id="point" class="sle2" name="point" type="number" min="0"
					max="2000" value="0"><br>
				<br>

				<button type="button" onclick="reservation();"
					style="width: 100%; height: 50px; background: #DE6B6A; color: white; border: 0px; font-size: 26px;">예약하기</button>
				<br>
				<div align="center">
					<label style="font-size: 15px;">예약시 보증금 </label><label id="pay"></label><label>원이
						결제되며, </label><br> <label style="font-size: 14px;">식사 후 결제될
						금액에서 빠지게 됩니다.</label> <input type="hidden" name="deposit" value=""
						id="deposit">
				</div>
			</div>
		</div>
		<input id="muid" name="muid" type="hidden" value=""> <input
			id="payprice" name="payprice" type="hidden" value="">
	</form>

	<script>
	var currentp = 0;
	$(document).ready(function(){
		currentp = $("#currentPoint").html() * 1;
	...
	<img id="eImg"
		style="position: absolute; left: 200px; top: 100px; width: 300px;">
	<script>
