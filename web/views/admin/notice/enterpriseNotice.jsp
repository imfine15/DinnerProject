...
<div style="width:80%; height:600px;margin-left: 230px; padding-top:40px; padding-left:30px;">
		...
		<div align="center" style="padding-left:10px; padding-right:10px; background: white; width:100%; padding-top: 30px; height: 80%;">
			...
			<button class="writebtn" onclick="location.href='<%=request.getContextPath()%>/views/admin/notice/enterpriseUploadNotice.jsp'">글쓰기</button>
		</div>
			<div class="pageingArea" align="center">
			<button
				onclick="location.href='<%=request.getContextPath()%>/selectElist.no?currentPage=1'"><<</button>
			...
			%>
			<button
				onclick="location.href='<%=request.getContextPath()%>/selectElist.no?currentPage=<%=currentPage + 1%>'">></button>
			<%
				}
			%>

			<button
				onclick="location.href='<%=request.getContextPath()%>/selectElist.no?currentPage=<%=maxPage%>'">>></button>
	</div>
		<script>
		$(function() {
			$("#listArea td").mouseenter(function() {
				...
				location.href="<%=request.getContextPath()%>/EntSelectOne.no?num="+ num;
							});
		});
	</script>
</body>
